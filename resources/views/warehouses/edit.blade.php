<x-layouts.app :title="__('messages.warehouses.edit_title') . ' · tekomata'">
    <x-slot:breadcrumbs>
        <x-breadcrumb :items="[
            ['label' => __('messages.nav.dashboard'),   'url' => route('dashboard')],
            ['label' => __('messages.nav.warehouses'),  'url' => route('warehouses.index')],
            ['label' => __('messages.warehouses.edit_title')],
        ]" />
    </x-slot:breadcrumbs>
    ...
    <div class="mx-auto w-full max-w-xl">
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
            <form method="POST" action="{{ route('warehouses.update', $warehouse['id']) }}" class="divide-y divide-gray-100">
                @csrf
                @method('PUT')
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <label for="is_active" class="block text-sm font-medium text-gray-700">
                                {{ __('messages.warehouses.active_label') }}
                            </label>
                            <p class="text-xs text-gray-400">{{ __('messages.warehouses.active_hint') }}</p>
                        </div>
                    </div>
                </div>
