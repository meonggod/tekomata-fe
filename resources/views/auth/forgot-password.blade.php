<x-layouts.public :title="__('messages.forgot.title') . ' · tekomata'">
    ...
    <div class="mx-auto flex w-full max-w-md flex-col py-10 sm:py-16">
        @if (session('sent'))
            {{-- Generic success state. Shown for every accepted request — it never
                 reveals whether the email belongs to a real account. --}}
            <div class="rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm">
                ...
                <h1 class="mt-5 text-xl font-semibold text-gray-900">{{ __('messages.forgot.check_email_title') }}</h1>
                <p class="mt-2 text-sm leading-relaxed text-gray-600">
                    {!! __('messages.forgot.check_email_body', ['email' => '<span class="font-medium text-gray-900">' . e(session('email', '')) . '</span>']) !!}
                </p>
                ...
                <a href="{{ route('password.request') }}" class="mt-4 inline-block text-sm font-medium text-indigo-600 hover:text-indigo-500">
                    {{ __('messages.forgot.different_email') }}
                </a>
            </div>
        @else
            <div class="rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
                <h1 class="text-2xl font-semibold tracking-tight text-gray-900">{{ __('messages.forgot.title') }}</h1>
                <p class="mt-1.5 text-sm text-gray-600">{{ __('messages.forgot.subtitle') }}</p>
                ...
                <form method="POST" action="{{ route('password.email') }}" class="mt-7 space-y-5" novalidate>
                    ...
                    <div>
                        <label for="email" class="block text-sm font-medium text-gray-700">
                            {{ __('messages.forgot.email_label') }}
                        </label>
                ...
                <p class="mt-6 flex items-center justify-center gap-1.5 text-xs text-gray-400">
                    ...
                    {{ __('messages.forgot.secure_note') }}
                </p>
            </div>

            <p class="mt-6 text-center text-sm text-gray-600">
                {{ __('messages.forgot.remembered') }}
